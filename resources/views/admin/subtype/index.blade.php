...
<div class="block-header">
   <a href="{{ route('admin.subtype.create') }}" class="btn btn-warning btn-lg waves-effect">
...
<tr>
    ...
    <td>{{ $subtype->updated_at }}</td>
    <td>
     <a class="btn btn-primary" href=""><i class="material-icons">remove_red_eye</i></a>
     <a class="btn btn-success" href="{{ route('admin.subtype.edit', $subtype->id) }}"><i class="material-icons">edit</i></a> 
     <button class="btn btn-danger waves-effect" type="button" onclick="deleteSubType({{ $subtype->id }})">
        <i class="material-icons">delete</i>
    </button>
    <form id="delete-form-{{ $subtype->id }}" action="{{ route('admin.subtype.destroy',$subtype->id) }}" method="POST" style="display: none;">
        @csrf
          @method('DELETE')
    </form>
